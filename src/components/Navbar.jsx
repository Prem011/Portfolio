...
const Navbar = () => {
  const [active, setActive] = useState(""); // Manage active state
  const [toggle, settoggle] = useState(false); // Manage mobile menu toggle

  return ( 
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`} >
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
        <Link to="/" className="flex justify-center gap-2" onClick={() => {
          setActive("");
          window.scrollTo(0, 0);
        }}>
